...
func TestNewCategoryController(t *testing.T) {
	...
	values := map[string][]string {
		"id" : []string { strconv.Itoa(blog_keyword.GetID()) },
	}
	...
	// Check for Blog-Posts 1
	assert.HTTPBodyContains(t, handler, "GET", "https://localhost"+ctrl_path , values, blog_post1.Title)
	assert.HTTPBodyContains(t, handler, "GET", "https://localhost"+ctrl_path , values, blog_post1.SubTitle)

	// Check for Blog-Posts 2
	assert.HTTPBodyContains(t, handler, "GET", "https://localhost"+ctrl_path , values, blog_post2.Title)
	assert.HTTPBodyContains(t, handler, "GET", "https://localhost"+ctrl_path , values, blog_post2.SubTitle)
}